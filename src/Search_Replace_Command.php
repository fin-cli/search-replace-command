...
use cli\Colors;
use cli\Table;
use FIN_CLI\Iterators;
use FIN_CLI\SearchReplacer;
use FIN_CLI\Utils;
use function cli\safe_substr;

class Search_Replace_Command extends FIN_CLI_Command {
	...
	public function __invoke( $args, $assoc_args ) {
		...
		if ( null !== $regex_limit ) {
			if ( ! preg_match( '/^(?:[0-9]+|-1)$/', $regex_limit ) || 0 === (int) $regex_limit ) {
				FIN_CLI::error( '`--regex-limit` expects a non-zero positive integer or -1.' );
			}
		...
		if ( ! empty( $this->regex ) ) {
			...
			if ( false === @preg_match( $search_regex, '' ) ) {
				...
				FIN_CLI::error( $msg . $preg_error_message );
			}
		}
		...
		if ( $old === $new && ! $this->regex ) {
			FIN_CLI::warning( "Replacement value '{$old}' is identical to search value '{$new}'. Skipping operation." );
			exit;
		}
		...
		if ( null !== $export ) {
			if ( $this->dry_run ) {
				FIN_CLI::error( 'You cannot supply --dry-run and --export at the same time.' );
			}
			if ( true === $export ) {
				...
				if ( false === $this->export_handle ) {
					$error = error_get_last();
					FIN_CLI::error( sprintf( 'Unable to open export file "%s" for writing: %s.', $export, $error['message'] ?? '(unknown error)' ) );
				}
			}
		...
		if ( null !== $log ) {
			if ( true === $log || '-' === $log ) {
				...
				if ( false === $this->log_handle ) {
					$error = error_get_last();
					FIN_CLI::error( sprintf( 'Unable to open log file "%s" for writing: %s.', $log, $error['message'] ?? '(unknown error)' ) );
				}
			}
			...
			$log_prefixes = getenv( 'FIN_CLI_SEARCH_REPLACE_LOG_PREFIXES' );
			...
			$log_colors = getenv( 'FIN_CLI_SEARCH_REPLACE_LOG_COLORS' );
			...
			$this->log_encoding = 0 === strpos( $findb->charset, 'utf8' ) ? 'UTF-8' : false;
		}
		...
		$tables = Utils\fin_get_table_names( $args, $assoc_args );
		...
		foreach ( $tables as $table ) {
			...
			if ( $this->export_handle ) {
				...
				$row = $findb->get_row( "SHOW CREATE TABLE $table_sql", ARRAY_N );
			...
			if ( empty( $primary_keys ) ) {
				...
				if ( $this->report ) {
					...
					FIN_CLI::warning( $all_columns ? "No primary keys for table '$table'." : "No such table '$table'." );
				}
				continue;
			}
			...
			foreach ( $columns as $col ) {
				...
				if ( $this->verbose && 'count' !== $this->format ) {
					...
					FIN_CLI::log( sprintf( 'Checking: %s.%s', $table, $col ) );
				}
				...
				if ( ! $php_only && ! $this->regex ) {
					...
					// phpcs:ignore FinPress.DB.PreparedSQL.InterpolatedNotPrepared -- escaped through self::esc_sql_ident
					$serial_row = $findb->get_row( "SELECT * FROM $table_sql WHERE $col_sql REGEXP '^[aiO]:[1-9]' LIMIT 1" );

					// When the regex triggers an error, we should fall back to PHP
					if ( false !== strpos( $findb->last_error, 'ERROR 1139' ) ) {
						$serial_row = true;
					}
				}
		...
		if ( FIN_CLI::get_config( 'quiet' ) || STDOUT === $this->export_handle ) {
			return;
		}

		if ( 'count' === $this->format ) {
			FIN_CLI::line( (string) $total );
			return;
		}
		...
		if ( ! $this->dry_run ) {
			if ( ! empty( $assoc_args['export'] ) ) {
				...
				if ( $total && 'Default' !== Utils\fin_get_cache_type() ) {
					$success_message .= ' Please remember to flush your persistent object cache with `fin cache flush`.';
				}
			}
			FIN_CLI::success( $success_message );
		} else {
			$success_message = ( 1 === $total ) ? '%d replacement to be made.' : '%d replacements to be made.';
			FIN_CLI::success( sprintf( $success_message, $total ) );
		}
	}
	...
	private function php_export_table( $table, $old, $new ) {
		...
		if ( $this->verbose && 'table' === $this->format ) {
			$this->start_time = microtime( true );
			FIN_CLI::log( sprintf( 'Checking: %s', $table ) );
		}
		...
		if ( $this->verbose && 'table' === $this->format ) {
			...
			FIN_CLI::log( sprintf( '%d columns and %d total rows affected using PHP (in %ss).', $total_cols, $total_rows, $time ) );
		}
	...
	private function sql_handle_col( $col, $primary_keys, $table, $old, $new ) {
		...
		if ( $this->dry_run ) {
			if ( $this->log_handle ) {
				...
				// phpcs:ignore FinPress.DB.PreparedSQL.InterpolatedNotPrepared -- escaped through self::esc_sql_ident
				$count = $findb->get_var( $findb->prepare( "SELECT COUNT($col_sql) FROM $table_sql WHERE $col_sql LIKE BINARY %s;", '%' . self::esc_like( $old ) . '%' ) );
			}
		} else {
			if ( $this->log_handle ) {
			...
			// phpcs:ignore FinPress.DB.PreparedSQL.InterpolatedNotPrepared -- escaped through self::esc_sql_ident
			$count = $findb->query( $findb->prepare( "UPDATE $table_sql SET $col_sql = REPLACE($col_sql, %s, %s);", $old, $new ) );
		}

		if ( $this->verbose && 'table' === $this->format ) {
			$time = round( microtime( true ) - $this->start_time, 3 );
			FIN_CLI::log( sprintf( '%d rows affected using SQL (in %ss).', $count, $time ) );
		}
		return $count;
	}
	...
	private function php_handle_col( $col, $primary_keys, $table, $old, $new ) {
		...
		$where_key          = '';
		if ( ! $this->regex ) {
			$base_key_condition = "$col_sql" . $findb->prepare( ' LIKE BINARY %s', '%' . self::esc_like( $old ) . '%' );
		...
		// phpcs:ignore
		while ( $rows = $findb->get_results( "SELECT {$primary_keys_sql} FROM {$table_sql} {$where_key} {$order_by_sql} LIMIT {$limit}" ) ) {
			foreach ( $rows as $keys ) {
				...
				// phpcs:ignore FinPress.DB.PreparedSQL.InterpolatedNotPrepared -- escaped through self::esc_sql_ident
				$col_value = $findb->get_var( "SELECT {$col_sql} FROM {$table_sql} WHERE {$where_sql}" );
				...
				if ( ! $this->dry_run ) {
					...
					$findb->update( $table, [ $col => $value ], $update_where );
				}
			}
		...
		if ( $this->verbose && 'table' === $this->format ) {
			$time = round( microtime( true ) - $this->start_time, 3 );
			FIN_CLI::log( sprintf( '%d rows affected using PHP (in %ss).', $count, $time ) );
		}
	...
	private function write_sql_row_fields( $table, $rows ) {
		...
		foreach ( $rows as $row_fields ) {
			...
			if ( ( 0 === $index % $export_insert_size && $index > 0 ) || $index === $count ) {
				...
				if ( method_exists( $findb, 'remove_placeholder_escape' ) ) {
					// since 4.8.3

					// @phpstan-ignore method.nonObject
					$sql = $findb->remove_placeholder_escape( $findb->prepare( $sql, array_values( $values ) ) ); // phpcs:ignore FinPress.DB.PreparedSQL.NotPrepared -- verified inputs above
				} else {
					// 4.8.2 or less
					// phpcs:ignore FinPress.DB.PreparedSQL.NotPrepared -- verified inputs above
					$sql = $findb->prepare( $sql, array_values( $values ) );
				}
	...
	private static function get_columns( $table ) {
		...
		$text_columns    = array();
		$all_columns     = array();
		$suppress_errors = $findb->suppress_errors();

		// phpcs:ignore FinPress.DB.PreparedSQL.InterpolatedNotPrepared -- escaped through self::esc_sql_ident
		$results = $findb->get_results( "DESCRIBE $table_sql" );
		...
		$findb->suppress_errors( $suppress_errors );
	...
	private static function esc_like( $old ) {
		...
		if ( method_exists( $findb, 'esc_like' ) ) {
			// 4.0
			$old = $findb->esc_like( $old );
		} else {
			// Note: this double escaping is actually necessary, even though `esc_like()` will be used in a `prepare()`.
			// @phpstan-ignore function.deprecated
			$old = like_escape( esc_sql( $old ) ); // phpcs:ignore FinPress.FIN.DeprecatedFunctions.like_escapeFound -- BC-layer for FIN 3.9 or less.
		}
	...
	private function get_colors( $assoc_args, $colors ) {
		$color_reset = FIN_CLI::colorize( '%n' );
		...
		foreach ( array_keys( $colors ) as $color_col ) {
			...
			if ( null !== $col_color_flag ) {
				if ( ! preg_match( $color_codes_regex, $col_color_flag, $matches ) ) {
					FIN_CLI::warning( "Unrecognized percent color code '$col_color_flag' for '{$color_col}_color'." );
				} else {
					$colors[ $color_col ] = $matches[0];
				}
			}
			$colors[ $color_col ] = $colors[ $color_col ] ? array( FIN_CLI::colorize( $colors[ $color_col ] ), $color_reset ) : array( '', '' );
		}
	...
	private function log_sql_diff( $col, $primary_keys, $table, $old, $new ) {
		...
		// phpcs:ignore FinPress.DB.PreparedSQL.InterpolatedNotPrepared -- escaped through self::esc_sql_ident
		$results = $findb->get_results( $findb->prepare( "SELECT {$primary_keys_sql}{$col_sql} FROM {$table_sql} WHERE {$col_sql} LIKE BINARY %s", '%' . self::esc_like( $old ) . '%' ), ARRAY_N );
